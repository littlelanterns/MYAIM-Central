...
-- Note: ON CONFLICT clause assumes there's a unique constraint on category_name
-- If not, the query will insert duplicates on re-run
-- To prevent duplicates without unique constraint, you can wrap in a check:

-- DO $$
-- BEGIN
--     IF NOT EXISTS (SELECT 1 FROM intention_categories WHERE category_name = 'family_relationships') THEN
