-- ============================================
-- MyAIM Central - Login System Migration
-- Version: 1.0
-- Date: 2025-10-18
...
ALTER TABLE families
  ADD COLUMN IF NOT EXISTS family_login_name TEXT UNIQUE;
...
-- Beta Users Table
-- Tracks users who have beta testing access
CREATE TABLE IF NOT EXISTS beta_users (
  ...
  -- Tier tracking
  beta_tier TEXT NOT NULL DEFAULT 'beta'
    CHECK (beta_tier IN ('alpha', 'beta', 'gamma')),
  ...
  -- Beta code (optional - for joining beta)
  beta_code TEXT,
...
CREATE INDEX IF NOT EXISTS idx_beta_users_beta_code ON beta_users(beta_code);
...
-- Controls admin access to different sections
...
-- Security: Track failed PIN attempts for rate limiting
...
-- Beta users can read their own record
CREATE POLICY "Beta users can read own record"
  ON beta_users FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

-- Super admins can read all beta users
...
-- Beta admins can read all beta users
...
-- Super admins can modify beta users
...
-- Super admins can view PIN attempt logs
...
-- NOTE: These will only work if the users exist in auth.users
DO $$
DECLARE
  admin_email TEXT;
  admin_user_id UUID;
BEGIN
  -- Loop through super admin emails
  FOR admin_email IN
    SELECT unnest(ARRAY[
  ...
  LOOP
    ...
    -- If user exists, grant super_admin permission
...
-- ============================================
-- PART 7: CONSTRAINTS & VALIDATIONS
-- ============================================
...
RETURNS TRIGGER AS $$
BEGIN
  -- If trying to set is_primary_parent = true
  IF NEW.is_primary_parent = true THEN
    -- Check if another member is already primary parent
    IF EXISTS (
...
BEGIN
  ...
  RAISE NOTICE '  1. Run this migration on Supabase';
  RAISE NOTICE '  2. Implement login pages';
