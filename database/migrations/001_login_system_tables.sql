-- ============================================
-- MyAIM Central - Login System Migration (CLEAN START)
-- Version: 1.0.2
-- Date: 2025-10-18
-- Purpose: Add all required tables and columns for 4-login-page system
-- This version handles partial migrations by cleaning up first
-- ============================================

-- ============================================
-- PART 0: CLEANUP (Safe to run even if nothing exists)
-- ============================================

-- Drop tables if they exist (CASCADE will drop dependent objects)
DROP TABLE IF EXISTS pin_attempt_log CASCADE;
...
DROP FUNCTION IF EXISTS log_pin_attempt(UUID, BOOLEAN, TEXT, TEXT);
DROP FUNCTION IF EXISTS unlock_pin_account(UUID);
DROP FUNCTION IF EXISTS update_beta_users_updated_at();
DROP FUNCTION IF EXISTS update_staff_permissions_updated_at();
DROP FUNCTION IF EXISTS enforce_single_primary_parent();
...
-- ============================================
-- PART 2: CREATE NEW TABLES (Clean slate)
-- ============================================
...
CREATE INDEX idx_beta_users_beta_code ON beta_users(beta_code) WHERE beta_code IS NOT NULL;
CREATE INDEX idx_beta_users_last_active ON beta_users(last_active);
...
CREATE INDEX idx_staff_permissions_permission_type ON staff_permissions(permission_type);
CREATE INDEX idx_staff_permissions_active ON staff_permissions(is_active, expires_at);
...
CREATE INDEX idx_pin_attempt_log_family_member_id ON pin_attempt_log(family_member_id);
CREATE INDEX idx_pin_attempt_log_attempted_at ON pin_attempt_log(attempted_at);
CREATE INDEX idx_pin_attempt_log_failed_recent
...
CREATE FUNCTION is_pin_locked(member_id UUID)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE FUNCTION log_pin_attempt(
  member_id UUID,
  success BOOLEAN,
...
CREATE FUNCTION unlock_pin_account(member_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
CREATE FUNCTION update_beta_users_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE FUNCTION update_staff_permissions_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE FUNCTION enforce_single_primary_parent()
RETURNS TRIGGER AS $$
BEGIN
...
BEGIN
  ...
  RAISE NOTICE 'Version: 1.0.2 (CLEAN)';
  ...
  RAISE NOTICE '  3. Test all 4 login pages';
  RAISE NOTICE '==============================================';
END $$;
